from polymeasure import PolyMeasure as M, library, FilterExpression, Rowset, Dimension
...
con = ddb.connect()
con.sql("create or replace table core as select * from 'NASA_Exoplanet_Composite.parquet'")

# Bind a library object to the core table/view -
...
# to_df(Core()).to_parquet("NASA_Exoplanet_Composite.parquet")
...
to_df(Core.median('sy_snum', where="pl_letter='b'", order_by='median_sy_snum'))
...
discoveries_by_year = to_df(Core(
    'byclass',
    outer=[
        discovery_subtypes,
        total_co_discovered
...
def get_prizes_measure(main_group, subgroups, where=None) -> (M, M):
    ...
    def summary_statistics(column) -> [M]:
        return [
            Core(f'{fn}_{column}', f"{fn}({column})") for fn in ('count', 'min', 'median', 'max', 'mean', 'var_pop')
        ]
...
print(Core(outer=Core.size(), dim=['disc_facility']).add_where(Core.W("disc_year=2023")).evaluate())
to_df(
    Core(
        outer=Core.size(),
        dim=['disc_facility'],
